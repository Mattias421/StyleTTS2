...
@click.option(
    '-p',
    '--config_path',
    default='Configs/exp7_dur_lstm/lstm.yml',
    type=str,
)
def main(config_path):
    ...
    if 'duration_lstm' in model:
        scheduler_params_dict['duration_lstm']['max_lr'] = config['optimizer_params'].get('lstm_lr', optimizer_params.ft_lr) * 2
    ...
    if 'duration_lstm' in model:
        acoustic_modules.append("duration_lstm")
    for module in acoustic_modules:
        for g in optimizer.optimizers[module].param_groups:
            g['betas'] = (0.0, 0.99)
            module_lr = config['optimizer_params'].get('lstm_lr', optimizer_params.ft_lr) if module == "duration_lstm" else optimizer_params.ft_lr
            g['lr'] = module_lr
    ...
    if 'duration_lstm' in optimizer.optimizers:
        print('duration_lstm', optimizer.optimizers['duration_lstm'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if 'duration_lstm' in model:
                lstm_durations = s2s_attn_mono.sum(axis=-1).detach()
