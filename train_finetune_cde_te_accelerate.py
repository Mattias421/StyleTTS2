...
@click.command()
@click.option('-p', '--config_path', default='Configs/config_ft_cde.yml', type=str)
def main(config_path):
    ...
    text_encoder_cde = getattr(model_params, 'text_encoder_cde', None)
    if text_encoder_cde is None:
        cde_config = getattr(model_params, 'cde', None)
    ...
    scheduler_params_dict['text_encoder']['max_lr'] = optimizer_params.ft_lr * 2
    ...
    if load_pretrained:
        if not config.get('load_only_params', True):
            logger.warning(
                'TextEncoderCDE changes the optimizer parameter layout; loading model '
                'parameters only and starting with a fresh optimizer.'
            )
